...
namespace internal {
	bool VideoCaptureInput::send_video_ = true;
...
bool VideoCaptureInput::EncoderProcess() {
  ...
  if (capture_event_.Wait(kThreadWaitTimeMs)) {
    ...
    {
      CriticalSectionScoped cs(capture_cs_.get());
      if (!captured_frame_.IsZeroSize() && send_video_) { // modified by jiarong
        deliver_frame = captured_frame_;
        captured_frame_.Reset();
      }
    }
    if (!deliver_frame.IsZeroSize() && send_video_) { // modified by jiarong
