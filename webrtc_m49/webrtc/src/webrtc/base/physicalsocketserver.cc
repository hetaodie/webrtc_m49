...
//jay add
    static std::string PhysicalSocket_EncodeAES( const std::string& password, const std::string& data,bool bCBC ,const std::string ivkey)
    {
        AES_KEY aes_key;
    ...
    static std::string PhysicalSocket_DecodeAES( const std::string& strPassword, const std::string& strData ,bool bCBC,const std::string ivkey)
    {
    ...
    std::string PhysicalSocket::udpAesKey_ = "";//"12345678901234567890123456789012";
    void PhysicalSocket::setUdpSocketEncryptKey(const std::string& aesKey) {
        udpAesKey_=aesKey;
    }
...
int PhysicalSocket::Send(const void* pv, size_t cb) {
 //   if( udpAesKey_.empty()==false) return cb;//added by wafer
...
int PhysicalSocket::SendTo(const void* buffer,
    ...
    if (udpEncrypt2_) {
        int len = (length > 12) ? 12 : length;
        char tmpBuffer[12];
        char *ptrBuffer = (char*)buffer;
        for (int i = 0; i < len; i++) {
            tmpBuffer[i] = ptrBuffer[len - i - 1];
        }
        memcpy(ptrBuffer, tmpBuffer, len);
    } else if ( udpAesKey_.empty()==false){
//        LOG(LS_INFO) << "++++++PhysicalSocket::SendTo+++++";
        ...
        buffer=encryptRtpData.data();
        length=encryptRtpData.size();
    }
...
int PhysicalSocket::RecvFrom(void* buffer,
  ...
  if (!success) {
    ...
    int size=length;
    if (udpEncrypt2_) {
        int len = (received > 12) ? 12 : received;
        char tmpBuffer[12];
        char *ptrBuffer = (char*)buffer;
        for (int i = 0; i < len; i++) {
            tmpBuffer[i] = ptrBuffer[len - i - 1];
...
//        LOG(LS_INFO) << "++++++PhysicalSocket::RecvFrom+++++";
        if(size>=16){
            ...
            memcpy((char*)buffer, (char*)stunData.data(), nsize);
            received=nsize;
        }else{
            //TODO, log
        }
