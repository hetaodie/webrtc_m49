...
static const int MIN_CHECK_RECEIVING_DELAY = 50;  // ms
    ...
    std::string P2PTransportChannel::connect_types_;
    std::vector<uint32_t> P2PTransportChannel::connect_times_;
    std::string P2PTransportChannel::s_route_path_;
    uint32_t P2PTransportChannel::lastConnectTime_ = 0;
    bool P2PTransportChannel::bConnectReady_ = false;
    bool P2PTransportChannel::bGetRouteFlag_ = true;
...
void P2PTransportChannel::UpdateState() {
  ...
  set_receiving(receiving);
    ...
    if (bConnectReady_ && writable && receiving) {
//        LOG_J(LS_INFO, this) << "+++++++++++++++++++++++++++++++++++P2PTransportChannel::UpdateState(): New best connection: " << best_connection_->ToString();
        
        if (connect_times_.size() < 5) {
            // local: 1, stun: 2, speedy relay: 3, coturn relay: 4
            char currentType;
            char s_priority[16];
            sprintf(s_priority, "%u", best_connection_->remote_candidate().priority());
            if (strstr(s_priority, "987654") != NULL) {
                currentType = '3';
            } else {
                const std::string& local_type = best_connection_->local_candidate().type();
                const std::string& remote_type = best_connection_->remote_candidate().type();
            ...
            if (connect_types_.empty()) {
                lastConnectTime_ = rtc::Time();
                connect_types_.append(1, currentType);
            } else if (currentType != connect_types_.back()) {
                uint32_t current_time = rtc::Time();
                uint32_t lastDuration = current_time - lastConnectTime_;
                if (lastDuration < 5000) {
                    connect_types_.pop_back();
                } else {
                    connect_times_.push_back(lastDuration / 1000);
                }
                connect_types_.append(1, currentType);
                lastConnectTime_ = current_time;
            }
        }
        
        bConnectReady_ = false;
    }

}

void P2PTransportChannel::clearRoute(){
    connect_times_.clear();
    connect_types_.clear();
    s_route_path_.clear();
    bConnectReady_ = false;
    lastConnectTime_ = 0;
    bGetRouteFlag_ = true;
...
void P2PTransportChannel::endCall() {
    if (connect_times_.size() < 5) {
        uint32_t current_time = rtc::Time();
        uint32_t lastDuration = current_time - lastConnectTime_;
        if (lastDuration >= 5000) {
    ...
    if (connect_types_.size() > 5) {
        connect_types_.pop_back();
    }
}

std::string P2PTransportChannel::getRoutePath() {
    if(!bGetRouteFlag_){
        return "";
    }
    endCall();
    for(unsigned long i = 0; i < connect_times_.size(); i++)
    {
        if (i > 0) {
            s_route_path_.append(";");
        }
        s_route_path_.append(1, connect_types_[i]);
        s_route_path_.append(":");
        char srt_time[16];
        sprintf(srt_time, "%u", connect_times_[i]);
        s_route_path_.append(srt_time);
    }
    bGetRouteFlag_ = false;
    return s_route_path_;
}
