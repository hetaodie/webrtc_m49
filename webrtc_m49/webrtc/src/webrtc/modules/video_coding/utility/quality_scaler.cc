...
//static const int kFramedropPercentThreshold = 60; // jiarong
...
const int QualityScaler::kDefaultMinDownscaleDimension = 80; // modified by jiarong

    // begin, add by jiarong
    int QualityScaler::start_downscale_shift_ = 1;
    int QualityScaler::max_downscale_shift_ = 2;
    int QualityScaler::min_downscale_shift_ = 1;
    int QualityScaler::lowQpThreshold_ = 25;
    int QualityScaler::frameDropPercentThreshold_ = 70;
    int QualityScaler::minQp_ = 22;
    int QualityScaler::maxQp_ = 45;
    // end
...
QualityScaler::QualityScaler()
    : num_samples_(0),
      low_qp_threshold_(-1),
//      downscale_shift_(0),
      downscale_shift_(start_downscale_shift_),
...
//  low_qp_threshold_ = low_qp_threshold;
  low_qp_threshold_ = QualityScaler::lowQpThreshold_; // modified by jiarong
...
void QualityScaler::OnEncodeFrame(const VideoFrame& frame) {
 ...
 //      avg_drop >= kFramedropPercentThreshold) ||
       avg_drop >= frameDropPercentThreshold_) ||
    ...
    if (use_framerate_reduction_ && !framerate_down_ && framerate_ >= 20) {
      ...
      if (downscale_shift_ < max_downscale_shift_) { // jiarong
        AdjustScale(false);
      }
    }
    ...
    if (use_framerate_reduction_ && framerate_down_) {
      ...
      if (downscale_shift_ > min_downscale_shift_) { // jiarong
        AdjustScale(true);
      }
    }
  }
  ...
  assert(downscale_shift_ >= 0);
    ...
    //lianwei add for limit min width and height:
    if(res_.width > res_.height)
    {
        if(res_.width < 240) {
            res_.width = 240;
            res_.height = 136;
