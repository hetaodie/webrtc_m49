...
public class WebRtcAudioTrack {//add public by wafer
  ...
  public static void SetObserver(Observer theObserver) {
    observer = theObserver;
  }
  public static interface Observer {
    public final int AudioTrack_Initialize_Failed = 0;
    public final int AudioTrack_Playstate_Not_Stopped = 1;
    public final int StreamType_Initialize_Failed = 2;

    public void onAudioTrackError(int errorCode);
  }
  // ********** end, add by wafer **********
...
/*      audioTrack = new AudioTrack(AudioManager.STREAM_VOICE_CALL,
      ...
      audioTrack = new AudioTrack(streamType,
                                  ...
                                  AudioFormat.ENCODING_PCM_16BIT,
                                  minBufferSizeInBytes,
                                  AudioTrack.MODE_STREAM); // modified by wafer
    ...
    if (audioTrack.getState() != AudioTrack.STATE_INITIALIZED) {
      if (observer != null)
        observer.onAudioTrackError(Observer.AudioTrack_Initialize_Failed);
      return;
    }

    if (audioTrack.getPlayState() != AudioTrack.PLAYSTATE_STOPPED) {
      if (observer != null)
        observer.onAudioTrackError(Observer.AudioTrack_Playstate_Not_Stopped);
      return;
    }

    if (audioTrack.getStreamType() != streamType) {
      if (observer != null)
        observer.onAudioTrackError(Observer.StreamType_Initialize_Failed);
      return;
    }
// modified by wafer
