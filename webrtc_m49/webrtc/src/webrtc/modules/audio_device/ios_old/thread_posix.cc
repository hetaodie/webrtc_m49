...
#include "webrtc/modules/audio_device/ios_old/event_wrapper.h"
...
ThreadPosix::ThreadPosix(ThreadRunFunction func, ThreadObj obj,
    ...
    : run_function_(func),
      obj_(obj),
      crit_state_(webrtc::CriticalSectionWrapper::CreateCriticalSection()),
      alive_(false),
      dead_(true),
      prio_(prio),
...
bool ThreadPosix::Start(unsigned int& thread_id)
{
  ...
  {
    webrtc::CriticalSectionScoped cs(crit_state_);
    dead_ = false;
  }
  ...
  if (kEventSignaled != event_->Wait(WEBRTC_EVENT_10_SEC)) {
    webrtc::WEBRTC_TRACE(webrtc::kTraceError, webrtc::kTraceUtility, -1,
...
void ThreadPosix::Run() {
...
#endif
    webrtc::WEBRTC_TRACE(webrtc::kTraceStateInfo, webrtc::kTraceUtility, -1,
  ...
  if (max_prio - min_prio > 2) {
    ...
    if (pthread_setschedparam(pthread_self(), policy, &param) != 0) {
      webrtc::WEBRTC_TRACE(
          webrtc::kTraceError, webrtc::kTraceUtility, -1, "unable to set thread priority");
    }
  }
